  ...
  this._transform = function _svgIcons2SVGFontStreamTransform(
    ...
    saxStream.on('opentag', function(tag) {
      ...
      try {
        ...
        d_arr[glyph.unicode]=tag.attributes.d;
    
        fill_arr[glyph.unicode]=tag.attributes.fill;
        
        stroke_arr[glyph.unicode]=tag.attributes.stroke;
        //console.log(tag.attributes.stroke + '\n');
        //console.log(glyph.unicode + ' : ' + d_arr[glyph.unicode] + '\n');        
        }
...
<defs>\n\
  <font id="' + options.fontId + '" horiz-adv-x="' + fontWidth + '">\n\
    ...
    var glyphName = ucs2.decode(unicode).map(function(point) {
        return 'uni0x' + point.toString(16).toUpperCase() + ';';
      }).join('');
    var unic = ucs2.decode(unicode).map(function(point) {
    ...
    <glyph glyph-name="' + glyphName + '"\n\
      unicode="' + unic + '"\n\
      horiz-adv-x="' + glyph.width + '" stroke-width="0.1"  opacity="1"  fill="' + fill_arr[unicode] + '" stroke="' + stroke_arr[unicode] + '"  d="' + d_arr[unicode] + '" transform="scale(1, -1) translate(0, -100)" />\n');
