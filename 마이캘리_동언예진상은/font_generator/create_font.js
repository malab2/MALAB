...
exec('python neural-fonts/train.py --experiment_dir=' + root_dir + ' --experiment_id=0 --batch_size=16 --lr=0.001 --epoch=1 --sample_steps=100 --schedule=20  --L1_penalty=100 --Lconst_penalty=15 --freeze_encoder=1', (error, stdout, stderr) => {		
    if (error)
      console.log(error);    
});
...
if (training_progress.length >= 4) {	
	...
	if (count == 1 && training_count == 0) {
		...
		exec('python neural-fonts/train.py --experiment_dir=' + root_dir + ' --experiment_id=0 --batch_size=16 --lr=0.001 --epoch=1 --sample_steps=100 --schedule=30  --L1_penalty=500 --Lconst_penalty=1000 --freeze_encoder=1', (error, stdout, stderr) => {
		if (error){
		  console.log(error);
		}
		});
